...
- [ ] **P3** The screen and host credentials trust whoever asks first, not that they're actually the TV/host  
  `claimOrValidateFirstClaim` (party/index.ts, shared by `claimOrValidateHost` and `claimOrValidateScreen`) accepts any non-empty client-supplied string and grants it on first use — there's nothing that distinguishes the real `/screen` tab (or the real host) from a player's own game tab. A player would have to deliberately open devtools and send a claim message by hand to squat either slot; this is a house game for friends, not an adversarial environment, so the realistic risk is low — but it's a real gap, not a hardened one. Once claimed there's also no re-claim path, so a deliberate (or accidental duplicate) claim locks the real screen/host out for the rest of the game. Host's practical exposure is lower than screen's: hostId is never transmitted anywhere but the host's own device (no QR/URL carries it — see the "manage as host" link in `app/room/[code]/screen/page.tsx`, private to the creating browser), whereas screenId is claimable by anyone who can reach the room. Real fix, if ever needed: a host-minted token for both. Originally found by `/ship`'s adversarial review on 2026-09-22 (screenId only); broadened to hostId by `/plan-eng-review` on 2026-09-22 when the connection-order host gate was removed. User reviewed and chose to ship as-is both times.
