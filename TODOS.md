...
- [ ] **P1** Any player can self-issue the screen credential and get the current round's video id directly  
  `claimOrValidateScreen` (party/index.ts) accepts any non-empty client-supplied string as the screen credential and grants it on first use — there is nothing that distinguishes an actual TV/projector from a player's own already-open game tab. A player can open devtools on their own connection and send `{"type":"GET_LYRICS_AUDIO","screenId":"x"}`; the reply goes directly back to them, revealing the current round's video id (the exact secret this whole host/screen split exists to protect), and `markPrivileged` then keeps that connection in `privilegedConns` for the rest of the game, so every future preview-phase deck (all answers, not just the current round) reaches them too. It also permanently locks out the legitimate screen, since `screenId` has no re-claim path once taken. This is a more severe framing than the "screen slot squatting" limitation documented alongside `claimOrValidateScreen`'s tests — that framing undersold it as a denial-of-service; the real impact is direct, permanent answer disclosure to any participant. Root cause: `screenId` conflates "a display was claimed first" with "this is actually the TV" — a player-writable string can't authenticate the latter. Real fix: a host-minted screen token (a new host-authorized message, e.g. `MINT_SCREEN_TOKEN`, callable only with a valid `hostId`; the QR link on `/host` embeds the minted token; `/screen` reads it from the URL instead of self-generating one via localStorage) — new message type, new client read-from-URL flow, updated QR generation, new tests. User explicitly chose to ship with this documented rather than build the fix now (2026-09-22). Found by `/ship`'s adversarial review on 2026-09-22.
