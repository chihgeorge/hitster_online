...
- [ ] **P2** T8: AI chat-to-diff editing on the standalone `/playlists` page  
  `docs/designs/decouple-quiz-bank.md`'s decision D4 — the "✨ Ask AI" chat box works in the room's editor (`PROPOSE_EDITS`/`PROPOSE_LYRIC_EDITS` over the room WebSocket) but not on `/playlists` (no room, no WebSocket). Needs a 3rd implementation: a stateless HTTP action on `party/playlist.ts` mirroring `handleProposeEdits`/`handleProposeLyricEdits`'s logic. Deliberately held back — user is redesigning the chat-to-diff UI itself, and building full parity before that redesign lands risks redoing this exact piece twice. Build once the redesign's final interaction model is locked. User decision, 2026-09-22.
